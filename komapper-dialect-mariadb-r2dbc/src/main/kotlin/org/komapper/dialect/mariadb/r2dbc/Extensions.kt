...
suspend fun <ENTITY : Any> R2dbcDatabase.createMissingProperties(table: Table<ENTITY>) {
    ...
    val indexes = getTableIndexes(table)
...
private suspend fun <ENTITY : Any> R2dbcDatabase.getTableColumns(table: Table<ENTITY>): List<String> {
    return runQuery {
        QueryDsl.fromTemplate("SELECT * FROM information_schema.columns WHERE table_schema = 'dbtest' AND table_name = '${table.tableName}'").select { row -> row.getNotNull<String>("column_name") }
    }
}
...
private suspend fun <ENTITY : Any> R2dbcDatabase.getTableIndexes(table: Table<ENTITY>): List<String> {
    val indexes = runQuery {
        QueryDsl.fromTemplate("SELECT * FROM information_schema.statistics WHERE table_schema = 'dbtest' AND table_name = '${table.tableName}' GROUP BY index_name").select { row -> row.getNotNull<String>("index_name") }
    }.toMutableList()
